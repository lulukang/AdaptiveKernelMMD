...
class egaussian(object):
    dimension=2
    mu=np.array([[0,2],[1.4,1.4],[2,0],[1.4,-1.4],[0,-2],[-1.4,-1.4],[-2,0],[-1.4,1.4]])
    mu=mu*2
    sigma=np.array([[0.2,0],[0,0.2]])
    inv_sigma =  np.array([[5,0],[0,5]])
    K=8
    def sample(self, n_samples):
        n= int(n_samples/8)
        x= np.zeros([8 * n, 2])
        for k in range(8):
            x[k*n:(k+1)*n, :] = np.random.multivariate_normal(self.mu[k, :], self.sigma, n)
        np.random.shuffle(x)
        return x
    ...
    def logp(self,x):
        ...
        Fx = np.zeros(n)
        for k in range(8):
            pdfi = multivariate_normal.pdf(x, mean=self.mu[k, :], cov=self.sigma)
            Fx += pdfi
        return np.log(Fx / 8)
    ...
    def grad_log_p(self, x):
        ...
        for k in range(8):
            pdfi = multivariate_normal.pdf(x, mean=self.mu[k, :], cov=self.sigma) + 1e-20
            Fx+= pdfi
            Jx+= pdfi[:, None] * np.matmul(self.mu[k, :] - x, self.inv_sigma)
        return Jx/Fx[:, None]    
